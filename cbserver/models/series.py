...
class Series(Base):
    ...
    def get_json_by_id(self):
        #series = db_session.query(Series).filter_by(id=self.id).all() or False
        issues = db_session.query(Issue).filter(Issue.series_id == self.series_id).limit(self.limit).offset(diff).all()

        #series_json = [dict({ key: value for key, value in list(series.__dict__.items()) if not key == "_sa_instance_state" }) for record in series]
        series_json = [dict({ key: value for key, value in list(row.__dict__.items()) if not key == "_sa_instance_state" }) for row in issues.getserieslist()]
        return series_json
