...
class ImageGetter(object):
    ...
    def extract_issue_cover(self):
        ...
        #Init, to get the first comic page from file.
        self.pagenum = 0 #The API may start at 1 but pagenum starts at 0. Careful.
        self.comicextractor() #Extracts the archive into files.
        result = self.read_page() #Grabs the first file/page.
        dest_base=(self.issuecoverpath + 'page0') #Using page0 to denote that the cover is extracted.
        cbmover = CBFile(source_path=result, issue=self.issue, dest_base=dest_base, id=self.id, size=self.size)

        #Split this up - function to gen name based on size, function to resize via Pillow.
        result = cbmover.move_cover()
        #cbmover.basepath=
        #cbmover.get_image_sizes()
        return result
