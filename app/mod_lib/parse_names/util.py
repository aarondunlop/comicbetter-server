...
def scancomics(folder):
    f = [(file, os.path.join(root, file)) for root, dir, files in os.walk(folder) for file in files]
    for comic, filename in f:
        found=False
        found=db.session.query(Issue).filter_by(filepath=filename).first()
        if not found:
